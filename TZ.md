...
**5. Внешние интеграции и аналитика**
 - Интеграция: планируется получение данных об объявлениях с Avito. Базовый сценарий - загрузка/обновление списка объявлений и фиксация цен (с сохранением ссылки, цены, региона, состояния при наличии).   
...
В ходе разработки был скорректирован способ получения данных с Avito. Изначально предполагалось использовать API площадки, однако Avito не предоставляет открытого API для свободного использования (доступ закрыт/ограничен и требует отдельных условий), поэтому прямое подключение API оказалось невозможным.
В качестве альтернативы был реализован собственный механизм получения данных (парсинг страниц/выдачи). Однако при развертывании на PythonAnywhere выяснилось, что регулярный парсинг/автоматические запросы к внешним сайтам ограничены условиями хостинга и могут приводить к блокировкам/ошибкам, из‑за чего данный модуль в продакшн-среде использовать нельзя. Поэтому для демонстрации функциональности на сервере данные по моделям/ссылки на поиск/изображения добавлялись вручную, а автоматический сбор данных оставлен для локального запуска/разработки.