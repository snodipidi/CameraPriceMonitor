...
def create_price_by_source_chart(df: pd.DataFrame, title: str = "Цены по источникам") -> str:
    ...
    for source in sources:
        ...
        color = colors.get(source, '#8b5a3c')
        ...
        fig.add_trace(go.Box(
            ...
            boxmean='sd',
            marker_color=color,
            line=dict(color='#3d2817', width=2),
        ))
...
def create_price_by_condition_chart(df: pd.DataFrame, title: str = "Цены по состоянию") -> str:
    ...
    df_plot = df.copy()
    df_plot['condition_label'] = df_plot['condition'].map(condition_labels).fillna(df_plot['condition'])
    
    fig = go.Figure()
    
    conditions = df_plot['condition_label'].unique()
    
    for condition in conditions:
        condition_data = df_plot[df_plot['condition_label'] == condition]['price']
